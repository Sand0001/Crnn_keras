...
#	print (img.shape)
	...
	#print('out',out)
	return out,y_pred_1.argmax(axis=2)[0],b
def del_blank(word):
        ...
        if "jpg" in input_image_path or 'png' in input_image_path:
            ...
            #for i in range(20):
                #img = Image.open(os.path.join(input_image_path,i).convert('L'))
                    ...
                    #print(predict(img))
                    #text,pred_arr = predict(img)
                    #print(text)
                    #print('len text_img_list',len(test_img_list))
                        ...
                        #print(label_text)
                        #print(text)
                        #np.save('../eng_test_subscript/pred_npy/'+i[:-4]+'.npy',pred_arr)
                        #script_label.writelines(i+'  ' + text + '\n')
